...
1. Create an `OwnerRepository`, an `IOwnerRepository` interface, and an `OwnersController` file and implement the `Index` and `Details` methods.
1. Update the `Startup.cs` to tell ASP.NET about the `OwnerRepository`.
